...
@section('title', 'Self Evaluation - PSCERMS')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-sm border p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800">Leadership Self-Evaluation Form</h1>
                <p class="text-gray-600 text-sm mt-1">Evaluate your own leadership performance</p>
                ...
                <div class="flex items-center space-x-4">
                    <div class="w-12 h-12 bg-green-600 rounded-full flex items-center justify-center">
                        <span class="text-white font-semibold text-lg">
                            {{ strtoupper(substr(auth()->user()->first_name ?? 'S', 0, 1)) }}{{ strtoupper(substr(auth()->user()->last_name ?? 'T', 0, 1)) }}
                        </span>
                    </div>
                    <div>
                        <h3 class="text-lg font-semibold text-gray-800">{{ auth()->user()->first_name ?? 'Sample' }} {{ auth()->user()->last_name ?? 'Student' }}</h3>
                        <p class="text-sm text-gray-600">Student ID: {{ auth()->user()->student_id ?? '2021-12345' }}</p>
